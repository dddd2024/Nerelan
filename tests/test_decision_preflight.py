...
def test_shared_pr_routing_both_workflows_checkout_exact_pr_head() -> None:
    """Transition validation must inspect the immutable PR head, never GitHub's synthetic merge ref."""

    expected_ref = "ref: ${{ github.event.pull_request.head.sha }}"
    for name in ("state-gate.yml", "decision-preflight.yml"):
        workflow = (REPO_ROOT / ".github" / "workflows" / name).read_text(encoding="utf-8")
        checkout_block = workflow.split("uses: actions/checkout@v4", 1)[1].split("      - name:", 1)[0]
