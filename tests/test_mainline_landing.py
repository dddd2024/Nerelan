...
# ---------------------------------------------------------------------------
# PR628 exact-head P0 production-shape correction: the commit-associated-PR
# list endpoint payload never contains a ``merged`` boolean key.  Resolution
# must pass on the real production shape (one associated PR, closed state,
# non-empty merged_at, matching merge_commit_sha, canonical repository) and
# must never require ``bool(pr["merged"])``.  BLOCK cases cover ambiguity,
# repository mismatch, missing merged_at, wrong merge_commit_sha, and malformed
# payloads.  Authoritative merged-state truth stays in the later
# ``verify_pr(..., require_merged=True)`` full-pull check, which this round
# never weakens.
# ---------------------------------------------------------------------------


def _resolve_merged_pull_request_via(
    payload: Any,
    *,
    repository: str = "dddd2024/Nerelan",
    merge_commit_sha: str = "c" * 40,
) -> dict[str, Any]:
    verifier = GitHubRemoteAcceptanceVerifier(repository=repository, token="test")
    verifier._request_json = lambda _path: payload  # type: ignore[method-assign]
    return verifier.resolve_merged_pull_request(merge_commit_sha=merge_commit_sha)


def _real_associated_pr(
    *,
    number: int = 628,
    repository: str = "dddd2024/Nerelan",
    merged_at: str = "2026-09-04T01:30:00Z",
    merge_commit_sha: str = "c" * 40,
    state: str = "closed",
) -> dict[str, Any]:
    return {
        ...
        "head": {"ref": "owner/issue156-postmerge-validator-cutover-r1", "sha": "9" * 40},
        "base": {
            "ref": "main",
            "sha": "8" * 40,
...
def test_resolver_accepts_real_production_shape_without_merged_key() -> None:
    pr = _real_associated_pr()
    assert "merged" not in pr
    result = _resolve_merged_pull_request_via([pr], merge_commit_sha="c" * 40)
    assert result["number"] == 628
    assert result["merged_at"] == "2026-09-04T01:30:00Z"
    assert result["merge_commit_sha"] == "c" * 40


def test_resolver_zero_associated_prs_blocks() -> None:
    with pytest.raises(GitHubEvidenceError, match="expected_one_merged_pr:observed=0"):
        _resolve_merged_pull_request_via([])


def test_resolver_multiple_associated_prs_block() -> None:
    payload = [_real_associated_pr(), _real_associated_pr(number=629)]
    with pytest.raises(GitHubEvidenceError, match="expected_one_merged_pr:observed=2"):
        _resolve_merged_pull_request_via(payload)


def test_resolver_wrong_repository_blocks() -> None:
    pr = _real_associated_pr(repository="other-owner/other-repo")
    with pytest.raises(GitHubEvidenceError, match="resolved_pr_repository_mismatch"):
        _resolve_merged_pull_request_via([pr])


def test_resolver_missing_merged_at_blocks() -> None:
    pr = _real_associated_pr(merged_at="")
    with pytest.raises(GitHubEvidenceError, match="resolved_pr_missing_merged_at"):
        _resolve_merged_pull_request_via([pr])


def test_resolver_wrong_merge_commit_sha_blocks() -> None:
    pr = _real_associated_pr(merge_commit_sha="d" * 40)
    with pytest.raises(GitHubEvidenceError, match="resolved_pr_merge_commit_mismatch"):
        _resolve_merged_pull_request_via([pr])


def test_resolver_open_state_blocks() -> None:
    pr = _real_associated_pr(state="open")
    with pytest.raises(GitHubEvidenceError, match="resolved_pr_not_closed"):
        _resolve_merged_pull_request_via([pr])


def test_resolver_malformed_payload_blocks() -> None:
    with pytest.raises(GitHubEvidenceError, match="expected_one_merged_pr:observed=invalid"):
        _resolve_merged_pull_request_via({"not": "a-list"})


def test_resolver_malformed_pr_object_blocks() -> None:
    with pytest.raises(GitHubEvidenceError, match="resolved_pr_shape_invalid"):
        _resolve_merged_pull_request_via(["not-a-dict"])


# ---------------------------------------------------------------------------
# False/none landing policy with a MISSING ``active_pr_binding_mode``: the
# policy classifier routes to ``cutover`` (because false implies no-legacy
# intent), but the final false/none gate must still BLOCK because the cutover
# authority contract requires ``active_pr_binding_mode == "none"``.  No
# authority bypass is ever granted by a missing binding-mode field.
# ---------------------------------------------------------------------------


def test_false_none_missing_binding_mode_final_gate_blocks(tmp_path: Path) -> None:
    bundle = _false_none_repo(
        tmp_path,
        contract_extra={"active_pr_binding_mode": None},
    )
    verifier, _ = _false_none_pair(bundle)
    result = _false_none_validate(bundle, verifier)
    assert result["gate_status"] == "BLOCKED", result
