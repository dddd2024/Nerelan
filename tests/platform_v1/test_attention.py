...
def test_complete_publication_is_draft_pr_ready_and_bound_url_is_exposed():
    read_model, _ = model(
        runs=[
            run(
                repository="Owner/Repo",
...
@pytest.mark.parametrize(
    ...
    [
        ("", "https://github.com/owner/repo/pull/123"),
        ("owner", "https://github.com/owner/repo/pull/123"),
        ("owner/../repo", "https://github.com/owner/repo/pull/123"),
        ("owner/repo\u200b", "https://github.com/owner/repo/pull/123"),
    ],
)
def test_invalid_projected_repository_never_authorizes_external_url(repository, url):
    read_model, _ = model(
        runs=[
            run(
                repository=repository,
...
def test_any_control_character_in_title_uses_fixed_fallback():
    read_model, _ = model(
