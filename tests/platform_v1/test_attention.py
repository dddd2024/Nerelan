from __future__ import annotations

from types import SimpleNamespace

import pytest

from reverse_agent.platform_v1.attention import (
    AttentionKind,
    AttentionReadModel,
    AttentionReadModelError,
    AttentionSeverity,
)
from reverse_agent.platform_v1.control_store import PlatformControlStore
from reverse_agent.platform_v1.run_store import TaskStore, TaskStoreError


class FakeControlStore:
    def __init__(self, goals=()):
        self.goals = list(goals)

    def list_goals(self, *, limit=100):
        return tuple(self.goals[:limit])


class FakeRunReadModel:
    def __init__(self, runs=()):
        self.runs = list(runs)
        self.requested_limits = []

    def list_runs(self, *, limit=100, cursor=None):
        self.requested_limits.append(limit)
        return {
            "runs": self.runs[:limit],
            "total": len(self.runs),
            "next_cursor": None,
        }


class FakeTaskStore:
    def __init__(self, repositories=None):
        self.repositories = dict(repositories or {})
        self.reads = []

    def get_task(self, task_id, *, event_limit=None):
        self.reads.append((task_id, event_limit))
        if task_id not in self.repositories:
            raise TaskStoreError(f"task_not_found:{task_id}")
        return SimpleNamespace(id=task_id, repository=self.repositories[task_id])


def goal(
    *,
    status="PLANNED",
    revision=1,
    goal_id="goal-1",
    title="Goal",
    updated_at="2026-09-12T08:00:00Z",
):
    return SimpleNamespace(
        id=goal_id,
        title=title,
        status=status,
        revision=revision,
        updated_at=updated_at,
    )


def run(
    *,
    task_id="task-1",
    goal_id="goal-1",
    title="Run",
    repository="projected/not-authoritative",
    status="RUNNING",
    liveness="ACTIVE",
    updated_at="2026-09-12T08:00:00Z",
    run_id="run-1",
    validation=None,
    publication=None,
):
    return {
        "task_id": task_id,
        "goal_id": goal_id,
        "title": title,
        "repository": repository,
        "status": status,
        "liveness": liveness,
        "updated_at": updated_at,
        "run_id": run_id,
        "validation": validation,
        "publication": publication,
    }


def model(*, goals=(), runs=(), task_repositories=None):
    fake_runs = FakeRunReadModel(runs)
    if task_repositories is None:
        task_repositories = {
            str(item.get("task_id", "")): "owner/repo"
            for item in runs
            if isinstance(item, dict) and item.get("task_id")
        }
    fake_store = FakeTaskStore(task_repositories)
    return (
        AttentionReadModel(
            store=fake_store,
            control_store=FakeControlStore(goals),
            run_read_model=fake_runs,
        ),
        fake_runs,
        fake_store,
    )


def complete_publication(url="https://github.com/owner/repo/pull/123", number=123):
    return {
        "status": "COMPLETE",
        "pr_number": number,
        "pr_url": url,
        "commit_sha": "a" * 40,
    }


def test_planned_goal_emits_approval_and_stable_identity():
    read_model, _, _ = model(goals=[goal()])
    first = read_model.list_items()
    second = read_model.list_items()
    assert len(first) == 1
    assert first[0].kind is AttentionKind.APPROVAL_REQUIRED
    assert first[0].severity is AttentionSeverity.MEDIUM
    assert first[0].deep_link == "/approvals?goal=goal-1"
    assert first[0].attention_id == second[0].attention_id


@pytest.mark.parametrize(
    "status", ["DRAFT", "APPROVED", "RUNNING", "COMPLETED", "INVALIDATED"]
)
def test_non_actionable_goal_states_are_not_attention(status):
    read_model, _, _ = model(goals=[goal(status=status)])
    assert read_model.list_items() == ()


def test_goal_revision_changes_attention_generation():
    first, _, _ = model(goals=[goal(revision=1)])
    second, _, _ = model(goals=[goal(revision=2)])
    assert first.list_items()[0].attention_id != second.list_items()[0].attention_id


@pytest.mark.parametrize(
    ("status", "expected_kind"),
    [
        ("FAILED", AttentionKind.RUN_FAILED),
        ("BLOCKED", AttentionKind.RUN_BLOCKED),
        ("INTERRUPTED", AttentionKind.RECOVERY_REQUIRED),
    ],
)
def test_terminal_or_interrupted_run_attention(status, expected_kind):
    read_model, _, _ = model(runs=[run(status=status, liveness="TERMINAL")])
    item = read_model.list_items()[0]
    assert item.kind is expected_kind
    assert item.severity is AttentionSeverity.HIGH


def test_active_stale_run_is_attention_but_healthy_active_run_is_not():
    stale, _, _ = model(runs=[run(status="RUNNING", liveness="STALE")])
    healthy, _, _ = model(runs=[run(status="RUNNING", liveness="ACTIVE")])
    assert stale.list_items()[0].kind is AttentionKind.RUN_STALLED
    assert healthy.list_items() == ()


@pytest.mark.parametrize("validation_status", ["FAILURE", "UNVERIFIED"])
def test_verification_failure_is_attention(validation_status):
    read_model, _, _ = model(
        runs=[
            run(
                status="VALIDATING",
                validation={
                    "command_id": "approved_functional_checks",
                    "status": validation_status,
                    "exit_code": 1,
                },
            )
        ]
    )
    item = read_model.list_items()[0]
    assert item.kind is AttentionKind.VERIFICATION_FAILED
    assert item.severity is AttentionSeverity.HIGH


def test_publication_failure_has_highest_run_precedence():
    read_model, _, _ = model(
        runs=[
            run(
                status="FAILED",
                liveness="STALE",
                validation={"status": "FAILURE", "exit_code": 1},
                publication={"status": "FAILED", "pr_number": 0, "pr_url": ""},
            )
        ]
    )
    items = read_model.list_items()
    assert len(items) == 1
    assert items[0].kind is AttentionKind.PUBLICATION_FAILED


def test_projected_repository_cannot_authorize_external_url():
    read_model, _, _ = model(
        runs=[
            run(
                repository="evil/other",
                status="READY_FOR_REVIEW",
                liveness="TERMINAL",
                publication=complete_publication(),
            )
        ],
        task_repositories={"task-1": "Owner/Repo"},
    )
    item = read_model.list_items()[0]
    assert item.kind is AttentionKind.DRAFT_PR_READY
    assert item.external_url == "https://github.com/owner/repo/pull/123"


@pytest.mark.parametrize(
    "unsafe_url",
    [
        "http://github.com/owner/repo/pull/123",
        "https://user:pass@github.com/owner/repo/pull/123",
        "https://evil.example/owner/repo/pull/123",
        "https://github.com/owner/repo/pull/123?token=secret",
        "https://github.com/owner/repo/pull/123#discussion",
        "https://github.com:444/owner/repo/pull/123",
        "https://github.com/settings/tokens",
        "https://github.com/other/repo/pull/123",
        "https://github.com/owner/repo/pull/124",
        "https://github.com/owner/repo/pull/123/files",
        "https://github.com/owner/repo/pull/123/",
        "https://github.com/owner/repo/pull/%31%32%33",
    ],
)
def test_unbound_or_unsafe_publication_url_is_never_exposed(unsafe_url):
    read_model, _, _ = model(
        runs=[
            run(
                status="READY_FOR_REVIEW",
                liveness="TERMINAL",
                publication=complete_publication(unsafe_url),
            )
        ]
    )
    item = read_model.list_items()[0]
    assert item.kind is AttentionKind.DRAFT_PR_READY
    assert item.external_url == ""


@pytest.mark.parametrize(
    "repository",
    ["", "owner", "owner/../repo", "owner/repo\u200b"],
)
def test_invalid_authoritative_repository_never_exposes_external_url(repository):
    read_model, _, _ = model(
        runs=[
            run(
                status="READY_FOR_REVIEW",
                liveness="TERMINAL",
                publication=complete_publication(),
            )
        ],
        task_repositories={"task-1": repository},
    )
    item = read_model.list_items()[0]
    assert item.kind is AttentionKind.DRAFT_PR_READY
    assert item.external_url == ""


def test_missing_authoritative_task_fails_closed_for_external_url():
    read_model, _, fake_store = model(
        runs=[
            run(
                status="READY_FOR_REVIEW",
                liveness="TERMINAL",
                publication=complete_publication(),
            )
        ],
        task_repositories={},
    )
    item = read_model.list_items()[0]
    assert item.kind is AttentionKind.DRAFT_PR_READY
    assert item.external_url == ""
    assert fake_store.reads == [("task-1", 0)]


@pytest.mark.parametrize("status", ["READY_FOR_REVIEW", "READY_FOR_REVIEW_FIXTURE"])
def test_ready_for_review_falls_back_to_review_attention(status):
    read_model, _, _ = model(runs=[run(status=status, liveness="TERMINAL")])
    item = read_model.list_items()[0]
    assert item.kind is AttentionKind.REVIEW_REQUIRED
    assert item.severity is AttentionSeverity.MEDIUM


def test_one_run_emits_only_highest_precedence_item():
    read_model, _, _ = model(
        runs=[
            run(
                status="FAILED",
                liveness="STALE",
                validation={"status": "FAILURE", "exit_code": 1},
                publication=complete_publication(),
            )
        ]
    )
    items = read_model.list_items()
    assert len(items) == 1
    assert items[0].kind is AttentionKind.RUN_FAILED


def test_high_sort_precedes_medium_then_newest_with_stable_tie_break():
    read_model, _, _ = model(
        goals=[goal(updated_at="2026-09-12T09:00:00Z")],
        runs=[
            run(task_id="task-old", status="FAILED", updated_at="2026-09-12T07:00:00Z"),
            run(task_id="task-new", status="BLOCKED", updated_at="2026-09-12T08:00:00Z"),
            run(
                task_id="task-review",
                status="READY_FOR_REVIEW",
                updated_at="2026-09-12T10:00:00Z",
            ),
        ],
    )
    items = read_model.list_items()
    assert [item.task_id for item in items[:2]] == ["task-new", "task-old"]
    assert all(item.severity is AttentionSeverity.HIGH for item in items[:2])
    assert all(item.severity is AttentionSeverity.MEDIUM for item in items[2:])


@pytest.mark.parametrize("limit", [0, -1, 101, True, "10"])
def test_invalid_limit_fails_closed(limit):
    read_model, _, _ = model()
    with pytest.raises(AttentionReadModelError, match="invalid_attention_limit"):
        read_model.list_items(limit=limit)


def test_source_reads_and_authoritative_task_reads_stay_bounded():
    runs = [run(task_id=f"task-{index}") for index in range(150)]
    repositories = {f"task-{index}": "owner/repo" for index in range(150)}
    read_model, fake_runs, fake_store = model(
        goals=[goal(goal_id=f"goal-{index}") for index in range(150)],
        runs=runs,
        task_repositories=repositories,
    )
    read_model.list_items(limit=100)
    assert fake_runs.requested_limits == [100]
    assert len(fake_store.reads) == 100
    assert all(event_limit == 0 for _, event_limit in fake_store.reads)


def test_sensitive_goal_title_and_run_fields_never_leak():
    sensitive = "Authorization Bearer sk-abcdefghijklmnop"
    read_model, _, _ = model(
        goals=[goal(title=sensitive)],
        runs=[run(task_id="task-secret", title=sensitive, status="FAILED")],
    )
    rendered = repr([item.to_dict() for item in read_model.list_items()])
    assert "sk-abcdefghijklmnop" not in rendered
    assert "Authorization" not in rendered
    assert "Goal needs approval" in rendered
    assert "Run needs attention" in rendered


@pytest.mark.parametrize(
    "obfuscated",
    [
        "pass\u200bword=hunter2",
        "sec\u2060ret=hunter2",
        "api\u0000_key=hunter2",
    ],
)
def test_control_obfuscated_titles_fail_closed_before_rendering(obfuscated):
    read_model, _, _ = model(
        goals=[goal(title=obfuscated)],
        runs=[run(task_id="task-secret", title=obfuscated, status="FAILED")],
    )
    rendered = repr([item.to_dict() for item in read_model.list_items()])
    assert "hunter2" not in rendered
    assert "Goal needs approval" in rendered
    assert "Run needs attention" in rendered


def test_projection_does_not_copy_untrusted_run_fields():
    data = run(status="FAILED")
    data.update(
        {
            "failure_detail": "password=super-secret",
            "events": [{"raw_log": "api_key=leak"}],
            "current_activity": {"description": "prompt secret"},
            "workspace": "/private/worktree",
        }
    )
    read_model, _, _ = model(runs=[data])
    rendered = repr(read_model.list_items()[0].to_dict())
    for forbidden in (
        "super-secret",
        "api_key=leak",
        "prompt secret",
        "/private/worktree",
    ):
        assert forbidden not in rendered


def test_real_store_projection_is_read_only_and_uses_existing_truth():
    store = TaskStore(":memory:")
    control = PlatformControlStore(store)
    task = store.create_task(title="Needs attention", repository="owner/repo")
    store.set_state(task.id, "FAILED")
    before = store._conn.total_changes

    items = AttentionReadModel(store=store, control_store=control).list_items()

    assert store._conn.total_changes == before
    assert len(items) == 1
    assert items[0].kind is AttentionKind.RUN_FAILED
    assert items[0].task_id == task.id


def test_real_planned_goal_projection_is_read_only():
    store = TaskStore(":memory:")
    control = PlatformControlStore(store)
    created = control.create_goal(
        title="Plan review",
        objective="objective-with-private-detail",
        repository="owner/repo",
        idempotency_key="attention-goal",
    )
    planned = control.save_goal_plan(
        created.id,
        expected_revision=created.revision,
        spec_markdown="spec",
        plan_markdown="plan",
        tasks=[{"id": "task-1", "title": "Do work", "dependencies": []}],
        acceptance_criteria=["passes"],
    )
    assert planned.status == "PLANNED"
    before = store._conn.total_changes

    items = AttentionReadModel(store=store, control_store=control).list_items()

    assert store._conn.total_changes == before
    assert len(items) == 1
    assert items[0].kind is AttentionKind.APPROVAL_REQUIRED
    assert "objective-with-private-detail" not in repr(items[0].to_dict())


def _real_published_attention(*, publication_repository="owner/repo", pr_url=None):
    store = TaskStore(":memory:")
    control = PlatformControlStore(store)
    task = store.create_task(
        title="Published run",
        repository="owner/repo",
        executor_kind="deterministic_fixture",
    )
    store.set_state(task.id, "RUNNING")
    store.set_state(task.id, "VALIDATING")
    store.set_state(task.id, "READY_FOR_REVIEW")
    control.upsert_publication(
        task_id=task.id,
        repository=publication_repository,
        base_branch="main",
        branch="codex/demo",
        status="COMPLETE",
        request_digest="attention-publication",
        commit_sha="a" * 40,
        pr_number=42,
        pr_url=pr_url or "https://github.com/owner/repo/pull/42",
    )
    before = store._conn.total_changes
    items = AttentionReadModel(store=store, control_store=control).list_items()
    assert store._conn.total_changes == before
    assert len(items) == 1
    return items[0]


def test_real_taskstore_runreadmodel_publication_exposes_exact_bound_url():
    item = _real_published_attention()
    assert item.kind is AttentionKind.DRAFT_PR_READY
    assert item.external_url == "https://github.com/owner/repo/pull/42"


def test_real_publication_wrong_repository_url_fails_closed():
    item = _real_published_attention(
        publication_repository="other/repo",
        pr_url="https://github.com/other/repo/pull/42",
    )
    assert item.kind is AttentionKind.DRAFT_PR_READY
    assert item.external_url == ""


def test_attention_identity_is_stable_for_same_authoritative_generation():
    runs = [
        run(
            task_id="task-1",
            status="READY_FOR_REVIEW",
            liveness="TERMINAL",
            publication=complete_publication(),
        )
    ]
    first, _, _ = model(runs=runs, task_repositories={"task-1": "owner/repo"})
    second, _, _ = model(runs=runs, task_repositories={"task-1": "owner/repo"})
    assert first.list_items()[0].attention_id == second.list_items()[0].attention_id
