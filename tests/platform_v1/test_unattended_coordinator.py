...
def _starvation_window(service: AutonomyService, *, policy_id: str, wip: int):
    now = datetime.now(timezone.utc)
    return service.activate({
        "policy_id": policy_id,
        "policy_revision": 1,
...
def test_cancel_hold_does_not_starve_later_runnable_task_when_wip_is_one(tmp_path):
    store = TaskStore(":memory:")
    control = PlatformControlStore(store)
    autonomy = AutonomyService(control_store=control, capabilities=CapabilityRegistry())
    window = _starvation_window(autonomy, policy_id="cancel-starvation-wip-one", wip=1)
    goals = GoalService(store=store, control_store=control)
    goal = _parallel_goal(goals, window.id, key="cancel-starvation-wip-one-goal", count=2)
    held_id, runnable_id = [link["task_id"] for link in control.list_goal_tasks(goal.id)]
    _prepare_cancelled_interrupted_task(store, held_id)
    assert control.runnable_tasks(window.id, limit=1) == (held_id,)
...
def test_multiple_cancel_holds_do_not_underfill_available_batch_capacity(tmp_path):
    store = TaskStore(":memory:")
    control = PlatformControlStore(store)
    autonomy = AutonomyService(control_store=control, capabilities=CapabilityRegistry())
    window = _starvation_window(autonomy, policy_id="cancel-starvation-wip-two", wip=2)
    goals = GoalService(store=store, control_store=control)
    goal = _parallel_goal(goals, window.id, key="cancel-starvation-wip-two-goal", count=4)
    task_ids = [link["task_id"] for link in control.list_goal_tasks(goal.id)]
    held_ids = task_ids[:2]
    runnable_ids = task_ids[2:]
    for held_id in held_ids:
        _prepare_cancelled_interrupted_task(store, held_id)
    assert control.runnable_tasks(window.id, limit=2) == tuple(held_ids)
    ...
    assert coordinator.tick() == 2
    assert set(calls) == set(runnable_ids)
    assert coordinator.status()["last_batch"]["size"] == 2
    for held_id in held_ids:
        assert store.get_task(held_id).status == "INTERRUPTED"
        assert control._conn.execute(
            "SELECT COUNT(*) AS c FROM platform_coordinator_claims WHERE task_id = ?",
    ...
    assert control.get_window(window.id).tasks_started == 2
