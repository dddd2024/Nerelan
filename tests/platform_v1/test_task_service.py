...
def test_goal_http_converges_to_blocked_on_failed_task(task_server) -> None:
    ...
    status, goal = _req(base, "POST", "/api/goals", {
        "objective": "HTTP failed convergence",
        "repository": "dddd2024/reverse-agent",
        "idempotency_key": "http-goal-fail-v1",
        "executor_kind": "deterministic_fixture",
        "orchestration_mode": "single",
    })
    assert status == 201
    ...
    now = datetime.now(timezone.utc)
    status, window = _req(base, "POST", "/api/windows/activate", {
        "policy_id": "http-fail-window", "policy_revision": 1, "owner_identity": "owner",
        "starts_at": (now - timedelta(seconds=1)).isoformat(),
        "expires_at": (now + timedelta(hours=1)).isoformat(),
        "repositories": ["dddd2024/reverse-agent"], "capabilities": ["execute_task"],
        "max_concurrent_tasks": 1, "max_tasks": 3, "max_retries": 0,
...
def test_goal_http_converges_to_blocked_on_blocked_task(task_server) -> None:
    ...
    status, goal = _req(base, "POST", "/api/goals", {
        "objective": "HTTP blocked convergence",
        "repository": "dddd2024/reverse-agent",
        "idempotency_key": "http-goal-blocked-v1",
        "executor_kind": "deterministic_fixture",
        "orchestration_mode": "single",
    })
    assert status == 201
    ...
    now = datetime.now(timezone.utc)
    status, window = _req(base, "POST", "/api/windows/activate", {
        "policy_id": "http-blocked-window", "policy_revision": 1, "owner_identity": "owner",
        "starts_at": (now - timedelta(seconds=1)).isoformat(),
        "expires_at": (now + timedelta(hours=1)).isoformat(),
        "repositories": ["dddd2024/reverse-agent"], "capabilities": ["execute_task"],
        "max_concurrent_tasks": 1, "max_tasks": 3, "max_retries": 0,
...
def test_goal_http_converges_to_blocked_on_cancelled_task(task_server) -> None:
    ...
    status, goal = _req(base, "POST", "/api/goals", {
        "objective": "HTTP cancelled convergence",
        "repository": "dddd2024/reverse-agent",
        "idempotency_key": "http-goal-cancelled-v1",
        "executor_kind": "deterministic_fixture",
        "orchestration_mode": "single",
    })
    assert status == 201
    ...
    now = datetime.now(timezone.utc)
    status, window = _req(base, "POST", "/api/windows/activate", {
        "policy_id": "http-cancelled-window", "policy_revision": 1, "owner_identity": "owner",
        "starts_at": (now - timedelta(seconds=1)).isoformat(),
        "expires_at": (now + timedelta(hours=1)).isoformat(),
        "repositories": ["dddd2024/reverse-agent"], "capabilities": ["execute_task"],
        "max_concurrent_tasks": 1, "max_tasks": 3, "max_retries": 0,
