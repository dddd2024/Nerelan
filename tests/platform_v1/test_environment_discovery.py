...
import reverse_agent.platform_v1.environment_discovery as environment_discovery
...
@pytest.mark.parametrize("parser", ["toml", "json"])
def test_parser_recursion_errors_are_sanitized(tmp_path, monkeypatch, parser: str) -> None:
    secret = "SECRET_SENTINEL_RECURSION_948"
    if parser == "toml":
        relative = "pyproject.toml"
        _write(tmp_path, relative, '[project]\nname = "fixture"\n')
        monkeypatch.setattr(
            environment_discovery.tomllib,
            "loads",
            lambda *_args, **_kwargs: (_ for _ in ()).throw(RecursionError(secret)),
        )
    else:
        relative = "package.json"
        _write(tmp_path, relative, '{"name":"fixture"}')
        monkeypatch.setattr(
...
def test_cargo_workspace_rust_version_inheritance(tmp_path) -> None:
    _write(
        tmp_path,
        "Cargo.toml",
        '[workspace]\nmembers = []\n[workspace.package]\nrust-version = "1.86"\n'
    ...
    rust = _runtime(result, "rust")
    assert [(item.constraint_kind, item.constraint) for item in rust] == [
        ("minimum", "1.86")
    ]
    assert rust[0].source.path == "Cargo.toml"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        '[package]\nname = "demo"\nrust-version.workspace = true\n',
        '[workspace]\n[workspace.package]\nrust-version = "1.86"\n'
        '[package]\nname = "demo"\nrust-version = { workspace = true, extra = true }\n',
    ],
)
def test_cargo_workspace_rust_version_invalid_forms_fail_closed(
    ...
    assert caught.value.code == "ENV_DISCOVERY_UNSUPPORTED_VALUE"
    assert caught.value.source == "Cargo.toml"


def test_go_directive_allows_trailing_line_comment(tmp_path) -> None:
    _write(
        tmp_path,
        "go.mod",
        "module example.test/demo\n\ngo 1.24.0 // toolchain baseline\n",
    )

    result = discover_environment_requirements(tmp_path)

    assert [(item.constraint_kind, item.constraint) for item in _runtime(result, "go")] == [
        ("minimum", "1.24.0")
    ]
...
def test_duplicate_go_directives_always_fail_closed(tmp_path, body: str) -> None:
    _write(tmp_path, "go.mod", "module example.test/demo\n" + body)
    ...
    assert caught.value.code == "ENV_DISCOVERY_SOURCE_INVALID"
    assert caught.value.source == "go.mod"
