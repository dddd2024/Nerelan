...
def test_out_of_order_now_cannot_resurrect_superseded_occurrence():
    store, control, _, service = _bundle()
    trigger = service.create(
        _payload(
            kind=FIXED_INTERVAL,
    ...
    newer = service.fire_due(trigger.id, now=BASE + timedelta(minutes=2))
    assert newer.outcome == CREATED
    assert newer.scheduled_for_utc == "2026-09-12T12:02:00Z"
    ...
    before_invocations = _count(store, "platform_goal_trigger_invocations")
    before_goals = _count(store, "platform_goals")

    stale = service.fire_due(trigger.id, now=BASE + timedelta(minutes=1))

    assert stale == newer
    assert store._conn.total_changes == before_changes
    assert _count(store, "platform_goal_trigger_invocations") == before_invocations
    assert _count(store, "platform_goals") == before_goals

    forward = service.fire_due(trigger.id, now=BASE + timedelta(minutes=3))
    assert forward.outcome == CREATED
    assert forward.scheduled_for_utc == "2026-09-12T12:03:00Z"
    assert forward.goal_id != newer.goal_id
    assert control.get_goal(forward.goal_id).status == "DRAFT"
