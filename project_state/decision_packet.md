...
```json decision_meta
{"schema_version":1,"decision_id":"decision_20260729_github_contents_base64_normalization_v4","round_id":"round_20260729_github_contents_base64_normalization_v4","based_on_state_build_id":"state_20260618_134029_d6bd033d2532","based_on_state_digest":"d6bd033d25324345cfd8ada0ac65db42bc86eb5017f3ffc92906fcd8b71cacb5","status":"APPROVED","mainline":"engineering_branch","skill_profiles":["reverse-agent-iteration@v2"]}
```

```json decision_contract
{
  "transition_kernel_required": true,
  "follows_last_decision_id": "decision_20260728_mainline_landing_semantic_rework_v3",
  "follows_last_round_id": "round_20260728_mainline_landing_semantic_rework_v3",
  "previous_audit_outcome": "REWORK_REQUIRED_BASE64_RESPONSE_NORMALIZATION",
  "workstream_id": "github-contents-base64-normalization-v4",
  "source_issue": 70,
  "predecessor_issue": 69,
  "active_pr": 67,
  "required_branch": "codex/pr60-mainline-landing-repair-v2",
  "starting_head": "65992a6e50b611ba6fcf6f5d3d1a36371bac9933",
  ...
  "capability_policy": {
    ...
    "local_network_exceptions": [
      ...
      "gh issue comment 70 --repo dddd2024/reverse-agent --body-file ISSUE_COMMENT_TEMP_PATH",
  ...
  "stop_conditions": [
    ...
    "base64_normalization_contract_cannot_be_preserved",
...
Authorize the bounded Issue #70 GitHub Contents Base64 normalization repair on
the existing branch and Draft PR #67. Preserve strict decoding and digest
verification while accepting only ASCII space, tab, CR, and LF formatting in
the GitHub Contents API Base64 representation.

### Authority and sequencing

This committed Decision is the v4 rework authority commit. The compiler-generated Command
Plan and successful `PRE_EXECUTION_AUTHORIZED` must be committed next. No source,
test, workflow, Gate runtime, schema, recovery record, MergeIntent, or
implementation mutation is authorized before
that gate succeeds.

The fixed starting head is
`65992a6e50b611ba6fcf6f5d3d1a36371bac9933`. No implementation or test change
...
in the structured contract are authorized. The implementation scope is limited
to `reverse_agent/github_remote_verifier.py` and
`tests/test_mainline_landing.py`; compiler-owned Gate artifacts are generated
authority only. Workflow, mainline validator, project Gate runtime, schemas,
recovery records, MergeIntent, dependencies, executor-neutral code, PR #47,
PR #49, and `main` remain read-only.

The decoder must require `encoding == "base64"` and string content, remove only
ASCII space, tab, CR, and LF, then use `base64.b64decode(..., validate=True)`.
Illegal characters, malformed padding, Unicode whitespace, unexpected encoding,
invalid content type, and digest mismatch must continue to fail closed.
