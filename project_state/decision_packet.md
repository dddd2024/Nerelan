# Decision Packet — #721 safe non-overlap base refresh current-main v7

```json decision_meta
{
  "schema_version": 1,
  "decision_id": "decision_20260918_issue721_safe_base_refresh_r2_v7",
  "round_id": "round_20260918_issue721_safe_base_refresh_r2_v7",
  "status": "APPROVED",
...
```json decision_contract
{
  ...
  "decision_scope": "SAFE_BASE_REFRESH_OWNER_LANDING_VALIDATION_R2_V7_CURRENT_MAIN",
  ...
  "previous_audit_outcome": "V5_LOCAL_VALIDATED_UNPUBLISHED_MAIN_DRIFT_EVIDENCE;V6_STALE_NO_AUTHORITY_EMPTY_BASELINE;ISSUE891_PR943_LANDED_MAIN_EF8BB695_WITH_STATE_GATE_3246_SUCCESS;V7_MUST_PRESERVE_891_READY_ROLE_SPLIT_AND_REMATERIALIZE_ONLY_SAFE_REFRESH_LOGIC",
  "workstream_id": "issue721-safe-base-refresh-r2-v7-current",
  "source_issue": 721,
  "parent_issue": 156,
  ...
  "required_branch": "owner/issue721-safe-base-refresh-r2-v7-current",
  ...
  "fresh_worktree_contract": {
    ...
    "target_branch": "owner/issue721-safe-base-refresh-r2-v7-current",
  ...
  "semantic_implementation_contract": {
    ...
    "specification": "Rematerialize the already locally validated v5 safe non-overlap base-refresh mechanism into current main while preserving all #891 false/none premerge attestation and Ready job role-split semantics. Reuse the existing OWNER_LANDING_MERGE_ATTESTATION family; extend it only when explicit refresh fields are present. Preserve strict locked-base behavior when refresh authority is absent. Validate exact full Git objects before ancestry/diff/merge-tree; require original base ancestor of refreshed base; require target/intervening path non-overlap; fail closed on authority-sensitive and dependency-manifest drift; bind unchanged target Decision digest; require deterministic merge-tree identity; recheck refreshed main/head CAS. No project_gate edit.",
    ...
    "reuse": [
      "current reverse_agent/mainline_landing.py",
      "validated v5 safe-refresh semantic logic as read-only content evidence",
      "existing GitHubRemoteAcceptanceVerifier",
      "system Git",
      "git merge-tree --write-tree",
      "fnmatch.fnmatchcase",
      "existing #891 premerge attestation and Ready-role split"
    ],
    "do_not_introduce": [
      "GitPython",
      "pygit2",
      ...
      "frontend changes",
      "project_gate changes"
    ],
    "validated_historical_semantic_head": "a8fa7ec1998b658311c7a7407dd73b521d19ced8",
    "no_history_reuse_no_cherry_pick_no_rebase_no_merge": true,
    "current_main_adaptation_required": true
  },
  ...
  "allowed_commands": [
    {
      "command_id": "issue721_r2v7.bootstrap",
      "command": "On exact current main in the distinct clean isolated v7 worktree, commit only this immutable Decision. Run startup-snapshot, transition-command-plan, transition-lint, transition-preflight --mode pre and worktree-publication-readiness. Generated gate files are ephemeral evidence only and must not be committed. Require PRE_EXECUTION_AUTHORIZED and PUBLICATION_READY before semantic mutation.",
      "phase": "bootstrap",
      "required": true,
      "expected_exit_codes": [
        0
    ...
    {
      "command_id": "issue721_r2v7.implement",
      "command": "After PRE_EXECUTION_AUTHORIZED, independently rematerialize the validated v5 safe-refresh logic into current mainline_landing.py and test_mainline_landing.py while preserving landed #891 semantics. Use v5 only as read-only content evidence; no cherry-pick/rebase/history transplant. Implement real fnmatch basename dependency matching, full N1-N10 fail-closed coverage, and a real #664 positive byte-binding migration proof. Commit exactly once.",
    ...
    {
      "command_id": "issue721_r2v7.validate",
      "command": "Run tests/test_mainline_landing.py, focused current #891 false_none regressions, governance transition/project-gate/decision-preflight tests, merge-intent tests and git diff --check. Re-run transition-command-plan/lint/preflight and publication readiness on exact semantic head. Natural exact-head CI/Decision Preflight/State Gate are mandatory final validation.",
      "phase": "validation",
      "required": true,
      "expected_exit_codes": [
        0
    ...
    {
      "command_id": "issue721_r2v7.publish",
      "command": "Through github_control_plane only create the exact Decision then semantic commit graph, branch owner/issue721-safe-base-refresh-r2-v7-current, and exactly one Draft PR against main@ef8bb6959ac37301c880e0971a97a9d56e9c99a9. No local git push, Ready or Merge.",
      "phase": "publication",
      "required": true,
      "expected_exit_codes": [
        0
    ...
    {
      "command_id": "issue721_r2v7.observe",
      "command": "Fresh-read exact base/head/changed paths and natural CI/Decision Preflight/State Gate, and Owner-audit the exact current-main safe-refresh integration including preservation of #891 semantics. Never Ready/Merge under this implementation Decision.",
  ...
  "capability_policy": {
    ...
    "github_control_plane_network_exceptions": [
      "Create only the v7 Decision/semantic commit graph and branch owner/issue721-safe-base-refresh-r2-v7-current in dddd2024/Nerelan, then exactly one Draft PR against main@ef8bb6959ac37301c880e0971a97a9d56e9c99a9; comment only on Issue721/156 and that Draft; never Ready/Merge."
    ]
  },
  "path_risk_floor": [
    {
  ...
  "run_environment_binding": {
    ...
    "target_owner_branch": "owner/issue721-safe-base-refresh-r2-v7-current",
...
Add a fail-closed non-overlap refreshed-base authority to the existing false/none Owner landing path without changing an independently accepted target head and without weakening the landed #891 pre-merge attestation lifecycle.

## Build vs reuse

Reuse the current landing machinery, GitHubRemoteAcceptanceVerifier, system Git, git merge-tree --write-tree, fnmatch.fnmatchcase and the locally validated v5 safe-refresh design. Do not add a merge scheduler, Git library, GitHub client, workflow, dependency, database or queue.

## Current-main coupling

PR #943 / Issue #891 is already landed on this exact base. Its ordinary state-gate vs formal landing-state-gate role split, existing-format attestation requirement, post-merge validation and project_gate behavior are read-only invariants in this round.

## Implementation scope

After PRE_EXECUTION_AUTHORIZED, exactly:
- reverse_agent/mainline_landing.py
- tests/test_mainline_landing.py

## Stop conditions

Stop on main/base drift, Decision mutation, source/test dirty before bootstrap, unavailable trusted-worker surface, failed preflight, scope expansion, failure of #891 regressions, N1-N10 or #664 positive proof, or exhausted publication budget. Generated gate evidence may be regenerated but never committed. No Ready/Merge under this Decision.
