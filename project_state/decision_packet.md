...
{
  "schema_version": 1,
  "decision_id": "decision_20260826_issue367_engineering_landing_boundary_r2_v5",
  "round_id": "round_20260826_issue367_engineering_landing_boundary_r2_v5",
  "status": "APPROVED",
...
```json decision_contract
{
  ...
  "previous_audit_outcome": "PR378_CLOSED_UNMERGED_PREMERGE_SCHEMA_DOWNGRADE",
  "workstream_id": "issue367-engineering-landing-boundary-r2-v5",
  "source_issue": 367,
  "parent_issue": 365,
  ...
  "required_branch": "owner/issue367-engineering-landing-boundary-r2-v5",
  "risk_tier": "R2",
  ...
  "allowed_commands": [
    {
      "command_id": "issue367_r2v5.bootstrap",
      "command": "verify the locked main base and fresh v5 branch; commit this v5 Decision as the unique first commit; run startup-snapshot, transition-command-plan, transition-lint, a normal write-result transition-preflight --mode pre, and worktree-publication-readiness; require PRE_EXECUTION_AUTHORIZED and PUBLICATION_READY; commit exactly the five initial generated governance artifacts as the first of at most two generated-governance commits before implementation",
      "phase": "bootstrap",
      "required": true,
      "expected_exit_codes": [
        0
    ...
    {
      "command_id": "issue367_r2v5.landing_authority_gate",
      "command": "verify the exact v4 source patch d3c2f9d87e94ed8c90a9f8c03d5406be6dc9096c applies cleanly to the locked base, replay it without committing, and preserve the complete accepted v4 source behavior; in the same single source commit change only reverse_agent/mainline_landing.py so validate_active_merge_intent first requires the raw schema_version to be a non-boolean integer exactly equal to 3 and returns a stable FAIL check named premerge_intent_schema_version otherwise, and validate_premerge_attestation applies the same raw schema-version requirement with stable FAIL check premerge_attestation_schema_version before invoking the canonical validators; do not change _validate_intent, _validate_attestation, validate_future_merge, post-merge routing, legacy schema compatibility, or any other v4 source behavior; do not add a second Gate, schema, intent, verifier, or authority store",
    ...
    {
      "command_id": "issue367_r2v5.landing_authority_tests",
      "command": "verify the exact v4 test patch 548c05290e100ddcbcfc27a2f5f5dc7b6d76de54 applies cleanly to the locked base and replay it without committing; retain the complete v4 regression matrix and add focused tests in tests/test_project_gate.py proving paired schema-1 and paired schema-2 active intent plus owner attestation are blocked by the corresponding new premerge schema-version checks rather than by incidental digest or workflow-profile failures; add focused tests in tests/test_mainline_landing.py proving schema-1 and schema-2 validate_future_merge post-merge compatibility remains PASSED; preserve all historical PR347 schema-v3, post-merge fail-closed, #364 Path-A lifecycle, #365 engineering-only blocking, and v4 side-effect coverage",
      "phase": "implementation",
      "required": true,
      "expected_exit_codes": [
        0
    ...
    {
      "command_id": "issue367_r2v5.validate_and_publish_draft",
      "command": "run the focused v5 schema-version nodes; run python -m pytest tests/test_project_gate.py tests/test_control_plane_transition.py tests/test_mainline_landing.py tests/platform_v1/test_merge_intent.py tests/test_ci_responsibility.py tests/test_path_a_gate.py -q; run the architecture/planning/risk/docs suite and the platform_v1 suite with only the four previously identified live OpenCode fake-provider tests deselected; run transition-lint, worktree-publication-readiness, git diff --check, and the write_result=False filename-and-byte/status side-effect proof; require all pass; push the exact branch for the first normal push and create exactly one replacement Draft PR against locked main; observe closed PR378 read-only only and do not reopen, comment, close, or resolve any review thread; do not mark Ready, merge, rerun workflows, or dispatch runners",
      "phase": "publication",
      "required": true,
      "expected_exit_codes": [
        0
    ...
    {
      "command_id": "issue367_r2v5.post_publication_binding",
      "command": "after the new v5 Draft PR yields its actual GitHub PR number, derive archive/pr347_v3.json only from the locked-base inherited PR347 active intent and prove byte-for-byte Git-blob identity; bind active.json exactly once to the new v5 PR using schema v3, raw 64-hex v4 Decision and Command Plan digests, locked base 9f5fa5a7c9846352346daf44c2d063bf8f6fb3bf, workflow_profile baseline, exact baseline workflows, merge method merge, equal_to_accepted_head_tree, and bounded expiry; commit only the two binding paths and perform the second normal push; neither issue 367 nor closed PR378 may substitute for the actual new v5 PR number",
  ...
  "replay_source_commit": "d3c2f9d87e94ed8c90a9f8c03d5406be6dc9096c",
  "replay_test_commit": "548c05290e100ddcbcfc27a2f5f5dc7b6d76de54",
  "cherry_pick_no_commit_limit": 2,
  "authorized_cherry_pick_no_commit_commits": [
    "d3c2f9d87e94ed8c90a9f8c03d5406be6dc9096c",
    "548c05290e100ddcbcfc27a2f5f5dc7b6d76de54"
  ],
