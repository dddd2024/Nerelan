...
{
  "schema_version": 1,
  "decision_id": "decision_20260816_issue223_opencode_auth_cardinality_r2_v1",
  "round_id": "round_20260816_issue223_opencode_auth_cardinality_r2_v1",
  "status": "APPROVED",
...
```json decision_contract
{
  ...
  "follows_last_decision_id": "decision_20260816_issue221_opencode_auth_probe_differential_r2_v1",
  "follows_last_round_id": "round_20260816_issue221_opencode_auth_probe_differential_r2_v1",
  "previous_audit_outcome": "ISSUE221_V1_BOUNDED_DIFFERENTIAL_COMPLETED_CARDINALITY_AND_IDENTITY_STILL_UNRESOLVED",
  "workstream_id": "issue223-opencode-auth-cardinality-r2-v1",
  "source_issue": 223,
  "parent_issue": 148,
  "required_branch": "owner/issue223-opencode-auth-cardinality-r2-v1",
  ...
  "external_harness_write_path": "F:/reverse-agent-issue223-opencode-auth-cardinality-v1/auth_cardinality_probe.py",
  "external_runtime_write_allowlist": [
    "F:/reverse-agent-issue223-opencode-auth-cardinality-v1/**"
  ],
  ...
  "capability_policy": {
    ...
    "local_network_exceptions": [
      ...
      "git -C F:/reverse-agent-planning-smoke fetch origin owner/issue223-opencode-auth-cardinality-r2-v1"
    ],
  ...
  "bootstrap_exception_commands": [
    ...
    "git -C F:/reverse-agent-planning-smoke rev-parse origin/owner/issue223-opencode-auth-cardinality-r2-v1",
    "powershell -NoProfile -Command \"$b=(git -C F:/reverse-agent-planning-smoke branch --list owner/issue223-opencode-auth-cardinality-r2-v1);if($b){'ISSUE223_V1_LOCAL_BRANCH_ALREADY_EXISTS';exit 25};if(Test-Path -LiteralPath 'F:/reverse-agent-issue223-opencode-auth-cardinality-r2-v1'){'ISSUE223_V1_AUTHORITY_WORKTREE_ALREADY_EXISTS';exit 24};if(Test-Path -LiteralPath 'F:/reverse-agent-issue223-probe-source-v1'){'ISSUE223_V1_SOURCE_WORKTREE_ALREADY_EXISTS';exit 23};if(Test-Path -LiteralPath 'F:/reverse-agent-issue223-opencode-auth-cardinality-v1'){'ISSUE223_V1_EVIDENCE_ROOT_ALREADY_EXISTS';exit 22};'ISSUE223_V1_BOOTSTRAP_TARGETS_ABSENT'\"",
    "git -C F:/reverse-agent-planning-smoke worktree add --track -b owner/issue223-opencode-auth-cardinality-r2-v1 F:/reverse-agent-issue223-opencode-auth-cardinality-r2-v1 origin/owner/issue223-opencode-auth-cardinality-r2-v1",
    "Set-Location F:/reverse-agent-issue223-opencode-auth-cardinality-r2-v1",
    "git status --short",
    "git rev-parse HEAD",
  ...
  "allowed_commands": [
    {
      "command_id": "issue223v1.source_worktree_create",
      "command": "git -C F:/reverse-agent-planning-smoke worktree add --detach F:/reverse-agent-issue223-probe-source-v1 3b650e6239336c796593cecd3c137cf839cf1e95",
      "phase": "setup",
      "required": true,
    ...
    {
      "command_id": "issue223v1.source_head",
      "command": "git -C F:/reverse-agent-issue223-probe-source-v1 rev-parse HEAD",
      "phase": "validation",
      "required": true,
      "expected_exit_codes": [0],
      "execution_surface": "local",
    ...
    {
      "command_id": "issue223v1.source_status_before",
      "command": "git -C F:/reverse-agent-issue223-probe-source-v1 status --short",
      "phase": "validation",
      "required": true,
      "expected_exit_codes": [0],
      "execution_surface": "local",
    ...
    {
      "command_id": "issue223v1.evidence_root_create",
      "command": "powershell -NoProfile -Command \"New-Item -ItemType Directory -Path 'F:/reverse-agent-issue223-opencode-auth-cardinality-v1' -ErrorAction Stop | Out-Null; 'ISSUE223_V1_EVIDENCE_ROOT_CREATED'\"",
      "phase": "setup",
      "required": true,
      "expected_exit_codes": [0],
      "execution_surface": "local",
    ...
    {
      "command_id": "issue223v1.auth_cardinality",
      "command": "powershell -NoProfile -Command \"$a=(git -C 'F:/reverse-agent-issue223-opencode-auth-cardinality-r2-v1' rev-parse HEAD).Trim(); python 'F:/reverse-agent-issue223-opencode-auth-cardinality-v1/auth_cardinality_probe.py' --source 'F:/reverse-agent-issue223-probe-source-v1' --root 'F:/reverse-agent-issue223-opencode-auth-cardinality-v1' --authority $a --planning '3b650e6239336c796593cecd3c137cf839cf1e95' --product-setup-target 'F:/reverse-agent/.platform_v1_runtime/model_setup_state.json'\"",
      "phase": "probe",
      "required": true,
    ...
    {
      "command_id": "issue223v1.source_status_after",
      "command": "git -C F:/reverse-agent-issue223-probe-source-v1 status --short",
      "phase": "validation",
      "required": true,
      "expected_exit_codes": [0],
    ...
    {
      "command_id": "issue223v1.authority_status_final",
      "command": "git -C F:/reverse-agent-issue223-opencode-auth-cardinality-r2-v1 status --short",
      "phase": "status",
      "required": true,
      "expected_exit_codes": [0],
      "execution_surface": "local",
...
`F:/reverse-agent-issue223-opencode-auth-cardinality-v1/auth_cardinality_probe.py`
...
6. For each executed auth command derive only: variant; return code; stdout/stderr nonempty; `credential_count`; `candidate_auth_row_count`; `exact_sensetime_api_row_present`.
7. `credential_count`: after ANSI/decor normalization, accept only whole lines matching `^\s*(\d+)\s+credentials?\s*$` case-insensitively. Across stdout/stderr all matches must agree on one integer; otherwise null. Never persist matched text.
8. `candidate_auth_row_count`: count normalized non-summary lines whose final token is one of `api`, `oauth`, `sso`, `account_login`, `external_cli_session`, `api_key`, `session`, `credential`. Never persist row text/provider labels.
9. `exact_sensetime_api_row_present`: true only when a candidate row has auth type exactly `api` and the complete provider-label portion equals `sensetime` case-insensitively after trimming. No fuzzy/substring/slug/display-name mapping.
10. Run location matrix only if inherited evidence is stronger than restricted: inherited count > 0 while restricted is 0/null, or inherited exact row true while restricted false. Fixed order: `USERPROFILE`, `HOME`, `LOCALAPPDATA`, `APPDATA`, `XDG_DATA_HOME`. Query each name directly; never enumerate env. Add at most one existing value per restricted trial; never display/persist/hash/measure/compare the value.
11. Harness may write only `F:/reverse-agent-issue223-opencode-auth-cardinality-v1/auth_cardinality_summary.json`, containing sanitized SHAs/CLI identity/observations/classification/false attestations. No raw terminal text, provider labels, credential rows/paths, location values, or full executable paths.
12. No model/provider call, `opencode run`, `opencode models`, Connection test, relay lease, Task execute, auth login/logout, Product Setup mutation, real TaskStore access, repo product/test mutation, or dependency install.
...
3. inherited credential_count == 0 -> `OPENCODE_INHERITED_CREDENTIAL_COUNT_ZERO`.
4. inherited and restricted exact `sensetime api` booleans both true -> `SENSITIME_AUTH_ROW_VISIBLE_IN_RESTRICTED_METADATA`.
5. inherited exact row true and restricted false -> run permitted location matrix; first restoring variable => `SENSITIME_AUTH_VISIBLE_WITH_LOCATION_ENV_<NAME>`; none => `SENSITIME_AUTH_VISIBLE_INHERITED_ONLY`.
6. inherited credential_count > 0 but exact `sensetime api` row false -> `CREDENTIALS_PRESENT_SENSITIME_IDENTITY_UNRESOLVED`.
7. otherwise -> `AUTH_LIST_CARDINALITY_UNRESOLVED`.
