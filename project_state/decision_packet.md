...
{
  "schema_version": 1,
  "decision_id": "decision_20260908_issue585_desktop0_tauri_r2_v8",
  "round_id": "round_20260908_issue585_desktop0_tauri_r2_v8",
  "status": "APPROVED",
...
```json decision_contract
{
  ...
  "decision_scope": "FRESH_MAIN_DESKTOP0_TAURI_RUNTIME_WIRING_REPAIR_R2_V8_POST663",
  "not_product_redesign": true,
  "follows_last_decision_id": "decision_20260908_issue585_desktop0_tauri_r2_v7",
  "follows_last_round_id": "round_20260908_issue585_desktop0_tauri_r2_v7",
  "previous_audit_outcome": "R2_V7_CLOSED_UNMERGED_AFTER_OWNER_EXACT_HEAD_AUDIT_FOUND_TAURI_DEV_PORT_OWNERSHIP_CONFLICT_ASYNC_FALSE_START_STOP_SUCCESS_AND_MISSING_DESKTOP_MODEL_CONTROL_BUILD_ROUTE",
  "workstream_id": "issue585-desktop0-tauri-native-startup-bridge-r2-v8",
  "source_issue": 585,
  "parent_issue": 357,
  ...
  "required_branch": "owner/desktop0-tauri-r2-v8",
  ...
  "v7_terminal_evidence": {
    "decision_commit": "68ab8d4249eedb141fb6f4ba3f3a1c72ee4b3c56",
    "semantic_head": "00462e1408858c205f2fb99a4f29b8adaaec8f1c",
    "pull_request": 663,
    "result": "CLOSED_UNMERGED_TERMINAL_NEGATIVE_PRODUCT_EVIDENCE",
    "blockers": [
      "TAURI_DEV_PORT_OWNERSHIP_CONFLICT",
      "START_STOP_FALSE_SUCCESS_FROM_CHILD_SPAWN",
      "MISSING_DESKTOP_MODEL_CONTROL_BUILD_ROUTE",
      "RUNTIME_READY_FROM_TCP_PROBE_ONLY"
    ],
    "root_cause": "the Tauri shell did not delegate lifecycle ownership to dev-up.ps1, did not wait for script exit status, did not bind the desktop build to the task-api and model-control base URLs, and treated loopback 4173 reachability as runtime readiness"
  },
  "v8_required_repairs": {
    "tauri_dev_port_ownership_conflict": "RESOLVED",
    "start_runtime_result_semantics": "DEV_UP_EXIT_VERIFIED",
    "stop_runtime_result_semantics": "DEV_DOWN_EXIT_VERIFIED",
  ...
  "fresh_worktree_contract": {
    ...
    "source_ref": "origin/owner/desktop0-tauri-r2-v8",
    "target_branch": "owner/desktop0-tauri-r2-v8",
  ...
  "draft_pr_contract": {
    "base": "main",
    "head": "owner/desktop0-tauri-r2-v8",
    "draft": true,
    "title": "R2 v8: repair DESKTOP-0 Tauri runtime wiring (#585)",
    ...
    "no_v8_final_evidence_requirement_for_tauri_ci_lane": true
  },
  ...
  "allowed_commands": [
    ...
    {
      "command_id": "issue585_desktop0_v8.update_package_json",
    ...
    {
      "command_id": "issue585_desktop0_v8.rust_toolchain_if_missing",
    ...
    {
      "command_id": "issue585_desktop0_v8.publish",
      "command": "after all blocking validation passes push the exact branch owner/desktop0-tauri-r2-v8 to locked main 5df08a23dbaa0e816f081b460ee6e2642004a2b1 and create exactly one Draft PR with body recording the immutable R2 authority snapshot plus PR #663 = CLOSED_UNMERGED_TERMINAL_NEGATIVE_PRODUCT_EVIDENCE and Issue #664 = DOWNSTREAM_TAURI_EXACT_HEAD_CI_OWNER; do not write Closes #585; do not claim a v8 Tauri CI final-evidence lane; never mark Ready and never merge under this Decision",
      "phase": "publication",
      "required": true,
      "expected_exit_codes": [0],
    ...
    {
      "command_id": "issue585_desktop0_v8.exact_head_acceptance",
      "command": "require a later read-only remote exact-head audit of the Draft PR head; Issue #664 owns the separate downstream Tauri exact-head CI lane and this Decision does not assert that a v8 Tauri CI lane already exists; keep the PR Draft and do NOT Ready or merge under this Decision",
  ...
  "forbidden_operations": [
    ...
    "claim_v8_tauri_ci_final_evidence_lane",
  ...
  "run_environment_binding": {
    ...
    "canonical_repository": "dddd2024/Nerelan",
    "target_owner_branch": "owner/desktop0-tauri-r2-v8",
...
Re-materialize the DESKTOP-0 Tauri runtime bridge on a fresh isolated worktree from `origin/owner/desktop0-tauri-r2-v8` at `5df08a23dbaa0e816f081b460ee6e2642004a2b1` and repair the three runtime defects the owner exact-head audit found in v7 PR #663, then publish exactly one Draft PR against `main`.

Required repairs:

1. **Tauri dev port ownership conflict — RESOLVED.** `dev-up.ps1` remains the single authoritative local stack lifecycle owner. `tauri.conf.json.beforeDevCommand` must not start an independent Vite on `4173`; it must invoke a Tauri-only helper under `frontend/src-tauri/scripts/dev-up-bridge.ps1` that delegates to `dev-up.ps1 -NoBrowser -RepoDir <trusted repo root>` and then waits for `http://127.0.0.1:4173`. `dev-up.ps1` and `dev-down.ps1` are not modified.
2. **False start/stop success — DEV_UP_EXIT_VERIFIED / DEV_DOWN_EXIT_VERIFIED.** `start_runtime` reports success only after `dev-up.ps1` exits with success following its own readiness checks; `stop_runtime` reports success only after `dev-down.ps1` exits with success. PowerShell invocation stays structured (`powershell.exe -NoProfile -NonInteractive -NoLogo -ExecutionPolicy Bypass -File <script> -NoBrowser -RepoDir <repo>`), never a concatenated command string. Output waiting is bounded and non-zero exit codes return sanitized failure information.
3. **Desktop model control routing.** A Tauri-only build helper under `frontend/src-tauri/scripts/desktop-build.ps1` sets `VITE_TASK_API_BASE=http://127.0.0.1:8766` and `VITE_MODEL_CONTROL_API_BASE=http://127.0.0.1:8765/api` before invoking the existing frontend build; `tauri.conf.json.beforeBuildCommand` uses that helper. `frontend/src/**` is not modified.
4. **Core-aware runtime status.** `runtime_status` observes the frontend (`127.0.0.1:4173`), the Task API (`127.0.0.1:8766`) and Model Control (`127.0.0.1:8765`) and exposes sanitized state; loopback `4173` reachability alone is never reported as runtime ready. No unknown process is terminated.

## Remaining limitation

This Decision does not authorize any product redesign, workflow mutation, launcher modification, test file mutation, `dev-up.ps1`/`dev-down.ps1` mutation, or any operation outside the `frontend/package.json`, `frontend/package-lock.json`, and `frontend/src-tauri/**` scope. The PR remains Draft until independent exact-head acceptance. No Ready, merge, tag, release, deployment, workflow rerun, or workflow dispatch is authorized under this Decision. Issue #664 separately owns the downstream Tauri exact-head CI lane; this Decision does not claim that a v8 Tauri CI lane already exists.
...
- The v7 Decision commit `68ab8d4249eedb141fb6f4ba3f3a1c72ee4b3c56` and semantic head `00462e1408858c205f2fb99a4f29b8adaaec8f1c` (PR #663) are terminal negative product evidence only. Do not amend, push, cherry-pick, rebase, merge, force-update, reopen, or reuse them; read them only through `git show`, `git diff`, and `git cat-file`.
...
- Do not execute the v2 through v7 worktrees; this round uses a fresh isolated worktree.
...
- All `execution_surface` values are from `CURRENT_AUTHORING_SURFACES` (`trusted_worker`, `github_control_plane`, `remote_observation`); the legacy `local` surface and `user_local` are never used.
