...
{
  "schema_version": 1,
  "decision_id": "decision_20260919_issue687_ci_diagnostic_isolation_r2_v1",
  "round_id": "round_20260919_issue687_ci_diagnostic_isolation_r2_v1",
  "status": "APPROVED",
...
```json decision_contract
{
  ...
  "required_branch": "codex/ci-diagnostic-tool-isolation-r2-20260919",
  "workstream_id": "issue687-ci-diagnostic-isolation-r2",
  "approved_by": "dddd2024 via explicitly delegated repository Owner execution",
  "approval_basis": "User renewed full Owner delegation to complete the final project goal. Existing Issue687 explicitly owns installed-tool test isolation and truthful nonblocking diagnostics. This bounded Decision authorizes these eight exact files only, with independent audit and separate landing.",
  "superseded_evidence": "None. PR954 and PR957 separately repair team/legacy fixtures; their source paths and branches are not changed here.",
  ...
  "capability_policy": {
    ...
    "github_control_plane_network_exceptions": [
      "Publish only locally authored exact Decision/semantic trees and commits to codex/ci-diagnostic-tool-isolation-r2-20260919 in dddd2024/Nerelan, one Draft against locked main, description/comments for Issue687 and that Draft. No local git push, Ready/Merge or dispatch."
    ]
  },
  ...
  "allowed_commands": [
    {
      ...
      "phase": "bootstrap",
      "command": "Use this exact Windows local checkout F:/Nerelan-issue687-ci-diagnostics. Commit Decision only, then startup-snapshot, transition-command-plan, transition-lint, transition-preflight --mode pre and worktree-publication-readiness; require PRE_EXECUTION_AUTHORIZED before test edits.",
      "required": true,
      "expected_exit_codes": [
        0
      ],
    ...
    },
    {
      ...
      "command": "Implement only the eight exact paths in semantic_implementation_contract. Preserve all four integration test method bodies and old blocking-deselection assertions. Commit once after scoped development checks; no real-tool test execution or product/runtime changes.",
      "required": true,
      "expected_exit_codes": [
        0
      ],
    ...
    },
    {
      ...
      "phase": "validation",
      "command": "Run python -B -m pytest tests/test_ci_responsibility.py tests/test_path_a_gate.py tests/test_control_plane_transition.py tests/test_decision_preflight.py -q -p no:cacheprovider; safe collect-only default and explicit-opt-in checks without running installed tests; verify exact4 marked node IDs and unchanged original integration test AST bodies; script summary fixtures include double failures/exit errors/missing and malformed XML; git diff --check; final transition-preflight/readiness. Natural full diagnostic must remove exactly four installed-tool failures against locked base and add none. Workflow/tests remain fail-closed. No actual installed-tool/provider calls.",
      "required": true,
      "expected_exit_codes": [
        0
      ],
...
## Scope
Implement existing Issue687 installed-tool selection and nonblocking diagnostic presentation with mature pytest/JUnit/GitHub mechanisms. No changes to production, fixture-repair branches, governance selector, blocking policy, dependencies or other workflows.
