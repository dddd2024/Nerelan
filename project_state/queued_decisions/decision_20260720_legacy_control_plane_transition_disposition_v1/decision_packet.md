...
```json queue_meta
{
  ...
  "candidate_path": "project_state/queued_decisions/decision_20260720_legacy_control_plane_transition_disposition_v1/decision_packet.md",
  "target_active_path": "project_state/decision_packet.md",
  "supersedes_candidate": "decision_20260719_mature_framework_adoption_control_plane_foundation_v1",
  "supersede_reason": "The previous candidate required an accepted legacy predecessor and attempted to extend the old control plane. The current user-authorized direction is to stop serial legacy closeout repair, freeze PR #5 as evidence, classify reusable capabilities, and select a clean migration baseline before framework implementation.",
  "activation_policy": "Do not execute from the queued path or from PR #6. Before activation, confirm PR #5 remains frozen at the audited exact head, synchronize current main, create a fresh execution branch from current main, promote this packet to project_state/decision_packet.md, generate a current command-plan, and record the activation commit before substantive work.",
  "estimated_effort_class": "large_goal_mode_governance_round",
  "effort_target_note": "Designed for approximately eight to twelve hours of bounded Codex work. Stop when the disposition artifacts and acceptance criteria are complete; do not pad work or reopen legacy gate repairs."
}
```
