...
OpenCode executor kwargs are built by exactly one helper
(`_build_executor_kwargs`), shared between the service and the historical
HTTP path.
...
The lock was required: the deterministic concurrency probe exposed
`sqlite3.InterfaceError` on the first attempt without it. With the lock,
the probe passes 20/20 stable runs.
