...
本文针对规划分支 `plan/framework-adoption-control-plane-v1`，给出一次以**整体架构切换**为主线的审计和下一步实现计划。
...
> 如何让项目从“Decision 驱动全部开发与执行”，切换为“GitHub Work Item 驱动工程任务、LangGraph 驱动工作流、BMAD 提供规划输入、reverse-agent Trust Layer 只控制高风险动作和二进制分析可信性”。
...
6. 测试依赖通过 `.[test]` 安装；
7. workflow cutover 后三个远端工作流能够成功。
...
下一步必须建立一个最小但真实可执行的架构主干，使以下链路第一次跑通：
...
| 二进制 Evidence、Claim、Counterevidence | reverse-agent Trust Layer | 不由 BMAD、GitHub 或 LangGraph 断言 |
...
当前 transition kernel 和 workflow cutover 已经完成迁移引导作用。它们应被保留，但不再继续扩展为项目总架构。
