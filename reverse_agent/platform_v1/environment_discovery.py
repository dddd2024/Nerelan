...
def discover_environment_requirements(
    ...
    if cargo_toml is not None:
        ...
        if package is not None and not isinstance(package, Mapping):
            raise _unsupported(source)
        version = _cargo_rust_version(payload, source)
...
def _cargo_rust_version(
    ...
    if package is not None and not isinstance(package, Mapping):
        raise _unsupported(source)
    if not isinstance(package, Mapping) or "rust-version" not in package:
        return None
    value = package["rust-version"]
    if isinstance(value, str):
        return _exact_version(value, "rust", source)
    if not isinstance(value, Mapping) or set(value) != {"workspace"} or value.get("workspace") is not True:
        raise _unsupported(source)
    workspace = payload.get("workspace")
    if not isinstance(workspace, Mapping):
        raise _unsupported(source)
    workspace_package = workspace.get("package")
    if not isinstance(workspace_package, Mapping) or "rust-version" not in workspace_package:
        raise _unsupported(source)
    return _exact_version(workspace_package["rust-version"], "rust", source)
