...
class CombinedTrustedHost:
    ...
    def stop(self) -> None:
        ...
        # Only call shutdown() for servers whose serve_forever loop actually
        # started. BaseServer.shutdown() blocks if serve_forever never ran,
        # which matters when start() fails after binding only some listeners.
        started_servers = servers[: len(self._threads)]
