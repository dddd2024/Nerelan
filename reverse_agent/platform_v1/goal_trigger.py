...
class GoalTriggerService:
    ...
    def fire_due(self, trigger_id: str, *, now: datetime) -> GoalTriggerInvocation:
        ...
        with self._write():
            ...
            high_water = self._latest_persisted_invocation(trigger.id)
            if (
                high_water is not None
                and high_water.scheduled_for_utc >= scheduled_for
            ):
                return high_water
    ...
    def _latest_persisted_invocation(
        self, trigger_id: str
    ) -> GoalTriggerInvocation | None:
        row = self._conn.execute(
            "SELECT * FROM platform_goal_trigger_invocations WHERE trigger_id = ? "
            "ORDER BY scheduled_for_utc DESC, occurrence_digest DESC LIMIT 1",
            (trigger_id,),
        ).fetchone()
        return None if row is None else self._row_to_invocation(row)
