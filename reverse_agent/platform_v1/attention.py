...
It derives bounded attention items from existing Goal, Task, Run, validation,
and publication truth. Repository identity is resolved from TaskStore rather
than trusting a RunReadModel fixture field.
"""
...
class AttentionReadModel:
    ...
    def list_items(
        self, *, limit: int = MAX_ATTENTION_ITEMS
    ) -> tuple[AttentionItem, ...]:
        ...
        run_page = self.run_read_model.list_runs(
            limit=min(MAX_RUNS, MAX_SOURCE_RECORDS)
        )
    ...
    def _goal_item(goal: Any) -> AttentionItem | None:
        ...
        return _build_item(
            ...
            generation={"status": status, "revision": revision},
        )

    def _authoritative_repository(self, task_id: str) -> str:
        """Resolve repository only from TaskStore truth.

        Missing Task truth fails closed to an empty repository. A projected
        ``run["repository"]`` field is deliberately ignored.
        """
        try:
            task = self.store.get_task(task_id, event_limit=0)
        except TaskStoreError:
            return ""
        return _safe_repository(getattr(task, "repository", ""))
    ...
    def _run_item(self, run: Mapping[str, Any]) -> AttentionItem | None:
        ...
        repository = self._authoritative_repository(task_id)
        ...
        if publication_status == "FAILED":
            ...
            source_status = publication_status
            external_url = _safe_github_pr_url(
                publication.get("pr_url", ""),
...
def _safe_title(value: Any) -> str:
    ...
    if _SENSITIVE_TEXT_RE.search(cleaned) or _OPAQUE_SECRET_RE.search(cleaned):
        return ""
...
def _safe_repository(value: Any) -> str:
    if not isinstance(value, str):
        return ""
    value = unicodedata.normalize("NFC", value.strip())
    if not 3 <= len(value) <= _MAX_REPOSITORY:
        return ""
    if any(unicodedata.category(char).startswith("C") for char in value):
        return ""
...
def _safe_github_pr_url(
    ...
    *,
    ...
    expected_path = f"/{owner}/{name}/pull/{number}"
