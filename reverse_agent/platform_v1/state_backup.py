...
def _unlink_if_identity(path: Path, identity: _FileIdentity | None) -> None:
...
def _atomic_link_install(temp_path: Path, final_path: Path) -> _FileIdentity:
    ...
    try:
        ...
        _unlink_if_identity(final_path, source_identity)
        raise
...
def _inspect_bound_database(
    ...
    try:
        ...
        _unlink_if_identity(stable_path, identity)
    ...
    def create_backup(
        ...
        with self._lock:
            ...
            try:
                ...
                database_identity = _atomic_link_install(
                    temp_database,
                    database_path,
                )
                temp_database = None
                manifest_identity = _atomic_link_install(
                    temp_manifest,
                    manifest_path,
                )
                temp_manifest = None
                ...
                _unlink_if_identity(database_path, database_identity)
                raise
            finally:
        ...
        *,
        ...
        try:
            ...
            if stable_manifest is not None:
                _unlink_if_identity(stable_manifest, manifest_identity)
