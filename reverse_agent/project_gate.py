...
# (FINAL_GATE_RESULT_NAME, REPORT_SUMMARY_RESULT_NAME, ROUND_DELTA_SUMMARY_NAME).
# ROUND_CLOSE_SNAPSHOT_RESULT_NAME and RUN_ROUND_RESULT_NAME are handled
# conditionally via round-matching logic and are not in this list.
...
def _command_phase(kind: str, *, archive_seen: bool) -> str:
    ...
    if kind == "archive-round":
        return "archive"
    if kind in {"final-check", "command-plan", "report-summary", "close-round", "run-round", "run-closeout", "gate-profile", "decision-lint", "execution-log", "report-auto-summary"}:
        return "gate"
    if kind in {
        "lint-report",
        "status",
...
def _command_expected_exit_codes(
    *,
    ...
    if kind in {"doctor", "lint-report", "report-summary", "final-check", "execution-log", "report-auto-summary"}:
    ...
    *,
    ...
    if execution_log_payload and isinstance(execution_log_payload.get("commands"), list):
        ...
        for entry in el_commands:
            ...
            if cmd and not _is_startup_command(cmd):
