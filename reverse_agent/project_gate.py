...
def _command_phase(kind: str, *, archive_seen: bool) -> str:
    ...
    if kind == "archive-round":
        return "archive"
    if kind in {"final-check", "command-plan", "report-summary", "close-round", "run-round", "run-closeout", "gate-profile", "decision-lint", "execution-log", "report-auto-summary", "execute-decision", "phase1-completion"}:
        return "gate"
    if kind in {
        "lint-report",
        "status",
