...
def execute_decision(
    *,
    ...
    """Thin decision-level entrypoint that delegates to run_round.
...
def naming_hygiene(*, state_dir: Path, repo_root: Path | None = None, write_result: bool = True) -> dict[str, Any]:
    ...
    # Read allowed state artifacts from decision contract
    decision_contract = read_decision_contract(state_dir) or {}
    allowed_state_artifacts = set(decision_contract.get("allowed_state_artifacts", []))
    ...
    for f in live_files:
        ...
        freshness_basis = "current_round" if category in ("current_live_artifact", "legacy_compat_artifact") else "unknown"
    ...
    inventory = {
        ...
        "no_neutral_live_path_created": True,
        "scan_scope": (
            "project_state/ immediate files and project_state/gates/ immediate JSON files. "
