...
def _hook_results_from_observations(observations: list[dict[str, object]]) -> dict[str, str]:
    ...
    has_compare_args = any(
        isinstance(item.get("compare_args"), dict) and item.get("compare_args") for item in observations
    )
    has_compare_entry = any(
        isinstance(item.get("compare_entry"), dict) and item.get("compare_entry") for item in observations
    )
    return {
        ...
        "compare_entry_args": "available" if has_compare_entry else "unavailable",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Hook samplereverse producer and actual compare helper entry")
    ...
    evidence = [
        f"compare_producer_trace_probe:target={target}",
        f"compare_producer_trace_probe:points={points_path}",
    ]
    ...
    if not target.exists():
        return _write_payload(
            ...
            _build_payload(
                success=False,
                summary="CompareProducerTraceProbe failed: target missing.",
                candidate_hex=args.probe_hex,
                hook_points=hook_points,
                evidence=[*evidence, "compare_producer_trace_probe:error=target_missing"],
    ...
    try:
        ...
        return _write_payload(
            ...
            _build_payload(
                success=False,
                summary="CompareProducerTraceProbe failed: missing frida or pywinauto.",
                candidate_hex=args.probe_hex,
                hook_points=hook_points,
                evidence=[*evidence, f"compare_producer_trace_probe:error={_escape_runtime_text(str(exc))}"],
                error=str(exc),
            ),
        )
...
function readWide(ptrValue, size) {{
    try {{
        ...
        let out = [];
        const limit = Math.min(Math.max(size, 10), 96);
...
function frameSlots(bp, moduleBase) {{
    let out = [];
    for (const offset of [0x1170, 0x116c, 0x1168, 0x1164, 0x1160]) {{
        try {{
            ...
            out.push({{
                ...
                preview_utf16le: readWide(value, 96),
            }});
        }} catch (error) {{
        }}
    }}
...
function callSiteCompareArgs(sp, moduleBase) {{
    let args = [];
    for (let i = 0; i < 3; i++) {{
        try {{
...
function compareEntryArgs(sp, moduleBase) {{
    let slots = [];
    for (let i = 0; i < 4; i++) {{
        try {{
            const slot = sp.add(i * Process.pointerSize);
            const rawValue = slot.readPointer();
            const preview = i > 0 && i < 3 ? readBytes(rawValue, 96) : "";
            slots.push({{
                index: i,
                esp_relative: "+0x" + (i * Process.pointerSize).toString(16),
                role: i === 0 ? "return_address" : (i === 1 ? "arg0" : (i === 2 ? "arg1" : "arg2_count")),
                value: rawValue.toString(),
                value_u32: rawValue.toUInt32(),
                module_offset: moduleOffsetText(rawValue, moduleBase),
                preview_hex: preview,
                preview_utf16le: i > 0 && i < 3 ? readWide(rawValue, 96) : "",
                looks_like_flag_target: preview.toLowerCase().startsWith("66006c00610067007b00"),
            }});
        }} catch (error) {{
        }}
    ...
    return {{
        convention: "callee entry stack: [esp+0]=return, [esp+4]=arg0, [esp+8]=arg1, [esp+0xc]=count",
        caller_return_address: slots.length > 0 ? slots[0].value : "",
        caller_return_module_offset: slots.length > 0 ? slots[0].module_offset : "",
        slots: slots,
    }};
}}
...
function observe(name, address, context) {{
    ...
    if (name === "wide_flag_prefix_compare") {{
        compareArgs = callSiteCompareArgs(sp, mainModule.base);
    }}
    if (name === "compare_helper_entry") {{
        compareEntry = compareEntryArgs(sp, mainModule.base);
    }}
    send({{
        type: "compare_producer_trace_observation",
        hook_name: name,
...
for (const point of hookPoints) {{
    try {{
        ...
        send({{ type: "compare_producer_trace_error", hook_name: String(point.name || ""), error: String(error) }});
    }}
}}
"""
    ...
    try:
        ...
        evidence.append(f"compare_producer_trace_probe:title={_escape_runtime_text(win.window_text() or '')}")
        evidence.append(f"compare_producer_trace_probe:probe_hex={args.probe_hex}")
        ...
        observations = [
            ...
            if str(payload.get("type", "")) == "compare_producer_trace_observation"
        ]
        hook_errors = [
            str(payload.get("error", ""))
            for payload in messages[before_count:]
            if str(payload.get("type", "")) == "compare_producer_trace_error"
        ]
        if hook_errors:
            evidence.extend(
                f"compare_producer_trace_probe:hook_error={_escape_runtime_text(item)}"
                for item in hook_errors
                if item
            )
        evidence.append(f"compare_producer_trace_probe:hook_observations={len(observations)}")
        return _write_payload(
            out_path,
            _build_payload(
                success=True,
                summary="CompareProducerTraceProbe completed producer and compare-entry audit.",
                candidate_hex=args.probe_hex,
                hook_points=hook_points,
                hook_observations=observations,
                evidence=evidence,
        ...
        evidence.append(f"compare_producer_trace_probe:error={_escape_runtime_text(str(exc))}")
        return _write_payload(
            out_path,
            _build_payload(
                success=False,
                summary="CompareProducerTraceProbe execution failed.",
                candidate_hex=args.probe_hex,
                hook_points=hook_points,
                evidence=evidence,
                error=str(exc),
