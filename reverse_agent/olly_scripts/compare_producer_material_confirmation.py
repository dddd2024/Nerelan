...
def _build_payload(
    *,
    ...
    confirmed_material_hook_candidates: list[dict[str, object]] | None = None,
    ...
    return {
        ...
        "confirmed_material_hook_candidates": confirmed_material_hook_candidates or [],
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Confirm upstream material source for samplereverse producer EAX")
    ...
    evidence = [
        f"compare_producer_material_confirmation:target={target}",
        f"compare_producer_material_confirmation:points={points_path}",
    ]
    ...
    if not target.exists():
        return _write_payload(
            ...
            _build_payload(
                success=False,
                summary="CompareProducerMaterialConfirmation failed: target missing.",
                candidate_hex=args.probe_hex,
                hook_points=hook_points,
                evidence=[*evidence, "compare_producer_material_confirmation:error=target_missing"],
    ...
    try:
        ...
        return _write_payload(
            ...
            _build_payload(
                success=False,
                summary="CompareProducerMaterialConfirmation failed: missing frida or pywinauto.",
                candidate_hex=args.probe_hex,
                hook_points=hook_points,
                evidence=[*evidence, f"compare_producer_material_confirmation:error={_escape_runtime_text(str(exc))}"],
                error=str(exc),
            ),
        )
...
function observe(point, address, context) {{
    ...
    send({{
        type: "compare_producer_material_confirmation_observation",
...
for (const point of hookPoints) {{
    try {{
        ...
        send({{ type: "compare_producer_material_confirmation_error", hook_name: String(point.name || ""), error: String(error) }});
    }}
}}
"""
    ...
    try:
        ...
        evidence.append(f"compare_producer_material_confirmation:title={_escape_runtime_text(win.window_text() or '')}")
        evidence.append(f"compare_producer_material_confirmation:probe_hex={args.probe_hex}")
    ...
    observations = [
        ...
        if str(item.get("type", "")) == "compare_producer_material_confirmation_observation"
    ]
    errors = [
        f"{item.get('hook_name', '')}:{item.get('error', '')}"
        for item in messages
        if str(item.get("type", "")) == "compare_producer_material_confirmation_error"
    ]
    success = bool(observations) and not script_errors
    summary = (
        "CompareProducerMaterialConfirmation captured bounded producer observations."
        if observations
        else "CompareProducerMaterialConfirmation did not capture bounded producer observations."
    )
    return _write_payload(
        out_path,
        _build_payload(
