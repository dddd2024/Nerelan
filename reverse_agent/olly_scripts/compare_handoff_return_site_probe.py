...
def _normalize_observation(item: dict[str, object] | None) -> dict[str, object]:
    ...
    return {
        ...
        "helper_enter_return_address_module_offset": str(item.get("helper_enter_return_address_module_offset", "")),
...
def _hook_results_from_observations(observations: list[dict[str, object]]) -> dict[str, str]:
    ...
    has_compare_args = any(isinstance(item.get("compare_args"), dict) and item.get("compare_args") for item in observations)
    has_return_site = any(str(item.get("hook_name", "")).startswith("helper_return_site_") for item in observations)
    return {
        ...
        "helper_return_site": "available" if has_return_site else "unavailable",
        ...
        "compare_call_args": "available" if has_compare_args else "unavailable",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Hook samplereverse compare handoff return site and compare args")
    ...
    evidence = [
        f"compare_handoff_return_site_probe:target={target}",
        f"compare_handoff_return_site_probe:points={points_path}",
    ]
    ...
    if not target.exists():
        return _write_payload(
            ...
            _build_payload(
                success=False,
                summary="CompareHandoffReturnSiteProbe failed: target missing.",
                candidate_hex=args.probe_hex,
                hook_points=hook_points,
                evidence=[*evidence, "compare_handoff_return_site_probe:error=target_missing"],
    ...
    try:
        ...
        return _write_payload(
            ...
            _build_payload(
                success=False,
                summary="CompareHandoffReturnSiteProbe failed: missing frida or pywinauto.",
                candidate_hex=args.probe_hex,
                hook_points=hook_points,
                evidence=[*evidence, f"compare_handoff_return_site_probe:error={_escape_runtime_text(str(exc))}"],
                error=str(exc),
            ),
        )
    ...
    for (let i = 0; i < 8; i++) {{
        try {{
            ...
            out.push({{
                ...
                preview_utf16le: readWide(value, 64),
            }});
        }} catch (error) {{
        }}
    }}
...
function compareArgs(sp, moduleBase) {{
    let args = [];
    for (let i = 0; i < 3; i++) {{
        try {{
            const slot = sp.add(i * Process.pointerSize);
            const rawValue = slot.readPointer();
            const preview = i < 2 ? readBytes(rawValue, 96) : "";
            args.push({{
                index: i,
                esp_relative: "+0x" + (i * Process.pointerSize).toString(16),
                value: rawValue.toString(),
                value_u32: rawValue.toUInt32(),
                module_offset: moduleOffsetText(rawValue, moduleBase),
                preview_hex: preview,
                preview_utf16le: i < 2 ? readWide(rawValue, 96) : "",
                looks_like_flag_target: preview.toLowerCase().startsWith("66006c00610067007b00"),
            }});
        }} catch (error) {{
        }}
    }}
    return {{
        ...
        args: args,
        lhs_ptr: args.length > 0 ? args[0].value : "",
        rhs_ptr: args.length > 1 ? args[1].value : "",
        count: args.length > 2 ? args[2].value_u32 : 0,
    }};
}}

function observe(name, address, context, extra) {{
    ...
    if (name === "wide_flag_prefix_compare") {{
        args = compareArgs(sp, mainModule.base);
        lhsPtr = safePointer(args.lhs_ptr || 0);
        rhsPtr = safePointer(args.rhs_ptr || 0);
        compareCount = Number(args.count || 0);
    }}
    const primaryLhs = name === "wide_flag_prefix_compare" ? lhsPtr : (slotPtr.isNull() ? esi : slotPtr);
    extra = extra || {{}};
    send({{
        type: "compare_handoff_return_site_observation",
        hook_name: name,
        ...
        return_address_module_offset: moduleOffsetText(returnAddress, mainModule.base),
        helper_enter_return_address: String(extra.helper_enter_return_address || ""),
        helper_enter_return_address_module_offset: String(extra.helper_enter_return_address_module_offset || ""),
...
function ensureReturnSiteHook(returnAddress, returnOffset) {{
    try {{
        if (!returnAddress || returnAddress.isNull()) {{
            return;
        }}
        const key = returnAddress.toString();
        if (returnSiteHooks[key]) {{
            return;
        }}
        returnSiteHooks[key] = true;
        Interceptor.attach(returnAddress, {{
            onEnter(args) {{
                observe("helper_return_site_" + String(returnOffset || "unknown"), returnAddress, this.context, {{}});
            }}
        }});
    }} catch (error) {{
        send({{ type: "compare_handoff_return_site_error", hook_name: "helper_return_site", error: String(error) }});
    }}
}}
        ...
        Interceptor.attach(address, {{
            onEnter(args) {{
                if (offset === helperOffset || name === "handoff_helper_enter") {{
                    ...
                    const retOffset = moduleOffsetText(ret, mainModule.base);
                    this.helperEnterReturnAddress = ret.toString();
                    this.helperEnterReturnOffset = retOffset;
                    ensureReturnSiteHook(ret, retOffset);
                }}
            ...
            onLeave(retval) {{
                if (offset === helperOffset || name === "handoff_helper_enter") {{
                    observe("handoff_helper_return", address, this.context, {{
                        helper_enter_return_address: this.helperEnterReturnAddress || "",
                        helper_enter_return_address_module_offset: this.helperEnterReturnOffset || "",
                    }});
                }}
            }}
        }});
    }} catch (error) {{
        send({{ type: "compare_handoff_return_site_error", hook_name: String(point.name || ""), error: String(error) }});
    }}
}}
"""
    ...
    try:
        ...
        evidence.append(f"compare_handoff_return_site_probe:title={_escape_runtime_text(win.window_text() or '')}")
        evidence.append(f"compare_handoff_return_site_probe:probe_hex={args.probe_hex}")
        ...
        observations = [
            ...
            if str(payload.get("type", "")) == "compare_handoff_return_site_observation"
        ]
        hook_errors = [
            str(payload.get("error", ""))
            for payload in messages[before_count:]
            if str(payload.get("type", "")) == "compare_handoff_return_site_error"
        ]
        if hook_errors:
            evidence.extend(
                f"compare_handoff_return_site_probe:hook_error={_escape_runtime_text(item)}"
                for item in hook_errors
                if item
            )
        evidence.append(f"compare_handoff_return_site_probe:hook_observations={len(observations)}")
        return _write_payload(
            out_path,
            _build_payload(
                success=True,
                summary="CompareHandoffReturnSiteProbe completed return-site and compare-argument audit.",
                candidate_hex=args.probe_hex,
                hook_points=hook_points,
                hook_observations=observations,
                evidence=evidence,
        ...
        evidence.append(f"compare_handoff_return_site_probe:error={_escape_runtime_text(str(exc))}")
        return _write_payload(
            out_path,
            _build_payload(
                success=False,
                summary="CompareHandoffReturnSiteProbe execution failed.",
                candidate_hex=args.probe_hex,
                hook_points=hook_points,
                evidence=evidence,
                error=str(exc),
