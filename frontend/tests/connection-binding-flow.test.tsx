...
describe("OpenCode Binding selection in NewTaskComposer", () => {
  ...
  it("deterministic_fixture can submit with no Binding and uses modelProfileId", async () => {
    const mockSubmit = vi.fn();
    ...
    await waitFor(() => {
      expect(screen.getByTestId("task-model-profile-select")).toBeInTheDocument();
    }, { timeout: 3000 });
