...
describe("repository selection for real-executor tasks", () => {
  ...
  it("fixture executor does not issue /api/repositories request", async () => {
    const mockSubmit = vi.fn();
    const qc = new QueryClient({
      defaultOptions: {
        queries: { retry: false, gcTime: 0, staleTime: Infinity },
    ...
    const repoModule = await import("@/lib/repository-client");
    const fetchReposSpy = vi.spyOn(repoModule, "fetchRepositories");

    renderWithProviders(
      <QueryClientProvider client={qc}>
        <NewTaskComposer open={true} onClose={() => undefined} onSubmit={mockSubmit} />
      </QueryClientProvider>,
    );

    await waitFor(() => {
      expect(qc.getQueryState(["repositories"])?.data).toBeDefined();
    }, { timeout: 3000 });

    const callsBeforeSwitch = fetchReposSpy.mock.calls.length;

    fireEvent.change(screen.getByTestId("task-title-input"), {
      target: { value: "fixture no repo fetch" },
    });
    ...
    expect(fetchReposSpy.mock.calls.length).toBe(callsBeforeSwitch);
