...
const mockRoadmapPhases: PlatformRoadmapPhase[] = [
  {
    ...
    goals: [
      { id: "goal-demo-platform", title: "完善无人值守多 Agent 平台", status: "RUNNING", repository: "dddd2024/Nerelan", updated_at: new Date(now.getTime() - 2 * 60_000).toISOString() },
      { id: "goal-demo-budget", title: "预算与成本硬上限", status: "PLANNED", repository: "dddd2024/Nerelan", updated_at: new Date(now.getTime() - 20 * 60_000).toISOString() },
    ],
...
export async function captureInboxItem(input: { title?: string; objective: string; repository?: string }): Promise<PlatformInboxItem> {
  if (isMock()) {
    ...
    const item: PlatformInboxItem = {
      ...
      objective: input.objective,
      repository: input.repository || "dddd2024/Nerelan",
      status: "CAPTURED",
      promoted_goal_id: "",
      created_at: timestamp,
      updated_at: timestamp,
