...
export function GoalProgress({ goal }: { goal: PlatformGoal }) {
  ...
  return (
    <section aria-label="执行进度" className="rounded-2xl border border-ra-border/70 bg-ra-workspace px-4 pt-4 pb-1 sm:px-5">
      ...
      {materialized && <p className="mb-2 rounded-lg bg-ra-light/60 px-3 py-2 text-xs leading-5 text-ra-text-secondary">
      ...
      <ol className="divide-y divide-ra-border/45">
        {links.map((task, index) => {
          ...
          return <li key={task.task_id} data-testid={`goal-task-${task.task_id}`} className="rounded-lg px-1 py-3 transition-colors hover:bg-ra-light/40">
            ...
            <div className="ml-7 mt-2 flex flex-wrap items-center gap-x-2 gap-y-1.5 text-[11px] leading-4">
              <span className={cn("rounded-md bg-ra-light px-1.5 py-0.5", taskStateClass(state))}>{materialized ? taskStateText(state) : "尚未启动"}</span>
              {materialized && <>
                <span data-testid={`goal-functional-status-${task.task_id}`} className={cn("rounded-md border border-ra-border/60 px-1.5 py-0.5", proof.verified ? "text-ra-status-running" : proof.status === "FAILED" ? "text-ra-status-error" : "text-ra-text-secondary")}>
