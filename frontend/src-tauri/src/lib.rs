...
fn build_dev_up_command(script: &Path, repo_root: &Path) -> Command {
...
fn build_dev_down_command(script: &Path, repo_root: &Path) -> Command {
    let mut command = Command::new("powershell.exe");
    command
        .arg("-NoProfile")
        .arg("-NonInteractive")
...
fn run_dev_up(script: &Path, repo_root: &Path) -> std::io::Result<ScriptOutcome> {
    run_lifecycle_command(build_dev_up_command(script, repo_root), START_TIMEOUT)
}

fn run_dev_down(script: &Path, repo_root: &Path) -> std::io::Result<ScriptOutcome> {
    run_lifecycle_command(build_dev_down_command(script, repo_root), STOP_TIMEOUT)
}
...
fn start_runtime() -> RuntimeStartResult {
    ...
    match run_dev_up(&script, &repo_root) {
...
fn stop_runtime() -> RuntimeStopResult {
    ...
    match run_dev_down(&script, &repo_root) {
