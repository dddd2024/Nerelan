...
| `src/components/features/sidebar/sidebar.tsx` | `Sidebar()` | `frontend/src/components/sidebar.tsx` | structurally-ported | OpenHands nav (Conversations, Customize, Automations, Settings) → reverse-agent nav (home, tasks, settings); removed BackendSelector/SettingsModal/env-switch; removed mobile drawer; kept 75px `md:flex-col` icon rail + `flex-row` mobile bar pattern | MIT |
